...
1. **Initial State** (`get_initial_state`):
   ...
   - The loader also restores hidden global neurons and their serialized 33-slot histories, voter histories, serialized party metadata/history rings, full per-voter party/sympathy inputs (`<milit>`, `<invotech>`, `<insocial>`, `<inliberal>`, `<oppsymp>`, `<playsymp>`, `<party>`, and `<orgs>`), policy runtime fields, delayed policy throttles, ministerial effectiveness, situation state, finance snapshots, and the `<inherited>` simvalue block (the values from two turns ago). Policy runtime distinguishes the current policy-neuron value (`<val>`) from the requested slider target (`<targ>`); the current value moves toward the target by a fixed `1 / implementation_time` step each turn. Hidden histories are advanced newest-first in snapshots; the native global-economy random cursor is not serialized, so its per-turn multiplier remains an explicit parity boundary. No per-node response calibration is applied; parity differences remain observable.
...
3. **End of Turn** (`process_end_of_turn`):
   ...
   - Advance the effect vector using the pre-turn policy values as source snapshots. Direct links use the current source throttle; inertial links shift a raw expression sample into their ring and average the leading window. A policy-owned ring retains its old head for the first process after a target change, then samples the pre-`Policy::NextTurn` value while the policy ramps. Saved rings contain raw samples, not minister-scaled live values.
      ...
      `SIM_LoadGame::LoadEffects` restores raw effect histories and input throttles, but not the desired output throttle for every outgoing effect. Consequently, arbitrary loaded saves can retain hidden ring-head state that cannot be reconstructed from the serialized policy or simvalue alone; the targeted Education/Health/OilSupply/WorkerProductivity residuals remain an explicit parity boundary.
