...
from autocracy.simulator import apply_actions, get_initial_state, process_end_of_turn
...
def test_uk_noop_corpus_contains_all_phase_zero_sections():
    ...
    assert case["initial_save"].endswith("gamedata/saves/uk0.xml")
    assert case["expected_save"].endswith("gamedata/saves/uk1.xml")
...
def test_uk_noop_ordered_core_has_a_credible_one_turn_error_budget():
    state, graph = get_initial_state("uk")
    actual = process_end_of_turn(state, graph)
    ...
    assert sum(error <= 0.01 for error in errors) >= 37
